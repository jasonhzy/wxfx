...
class PageController extends Controller{
	...
	function index(){
		...
		$this->action('common','checkjump');
        $this->action('shopping','pay_successs_tatus2',array('order_sn'=>'20151430118662','status'=>1));
		$this->js(array('jquery.json-1.3.js','goods.js?v=17'));//将js文件放到页面头
		//轮播js css
		$this->css(array("flexslider.css"));
		$this->js(array("jquery.flexslider-min.js","main.js"));
	 	...
	 	//if(is_null($rt)) {
		   //分类产品
			...
			if(!empty($rt['cat']))foreach($rt['cat'] as $row){
				...
				$cid = $row['cat_id'];
				$sql = "SELECT goods_id,goods_name,goods_thumb,goods_img,pifa_price,shop_price FROM `{$this->App->prefix()}goods` WHERE cat_id ='$cid' AND is_on_sale='1' AND is_jifen = '0' AND is_delete = '0' AND (is_best ='1' OR is_new='1' OR is_hot='1') ORDER BY sort_order ASC LIMIT 6";
